...
class Scoreboard (implicit  p: Parameters) extends XSModule with HasXSParameter {
  ...
  val commitVec = Wire(Vec(2, Vec(32, Bool())))
  for (i <- 0 until 32) {
  ...
  val commitVecUInt = Wire(Vec(2, UInt(32.W)))
  val real_commitVec = Wire(Vec(2, Vec(32, Bool())))
  real_commitVec.foreach(_.foreach(_ := false.B))
  for (i <- 0 until 2) {
    commitVecUInt(i) := commitVec(i).asUInt
  }

  val selBits = Seq.tabulate(2)(i => Seq.tabulate(32)(j => Mux(j.U >= readPtr, true.B, false.B)))
  val shiftedIndices = Wire(Vec(2, Vec(32, UInt(5.W))))
  for (i <- 0 until 2) {
    for (j <- 0 until 32) {
  ...
  for (i <- 0 until 32) {
    ...
    for (j <- 0 until i) {
      m_wawMatchVec(j) := rd_array(j) === rd_array(i) && (OpType_array(i) === MATUOpType.mtest || OpType_array(i) === MATUOpType.mmul) && (state_array(j) === s_wait || state_array(j) === s_commit) &&
        (state_array(i) === s_commit || state_array(i) === s_wait) && ((OpType_array(j) === LSUOpType.mld && offset_array(i) === offset_array(j)) || (OpType_array(j) === MATUOpType.mmul ||
  ...
  io.ldOut.offset_out := offset_array(PriorityEncoder(ld_wr_en_vec))
